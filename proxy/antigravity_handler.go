...
func ForwardToAntigravity(client *http.Client, upstream config.Upstream, requestBody []byte, clientWantsStream bool) (int, []byte, http.Header, error) {
	...
	return forwardWithAuthRetry(authRetryConfig[AntigravityTokenStorage]{
		Label:   "[ANTIGRAVITY]",
		Manager: antigravityAuthManager,
		Validate: func(storage *AntigravityTokenStorage, authFile string) error {
			if storage.ProjectID == "" {
				return fmt.Errorf("project_id is empty for auth file %s", authFile)
			}
			return nil
		},
		MarshalBody: func(base map[string]any, storage *AntigravityTokenStorage) ([]byte, error) {
			payload := clonePayload(base)
			payload["project"] = storage.ProjectID
			payload["requestId"] = "agent-" + uuid.NewString()
			return json.Marshal(payload)
		},
		BuildReq: func(u config.Upstream, body []byte, token string, _ *AntigravityTokenStorage, ctx context.Context) (*http.Request, error) {
...
func ForwardToAntigravityStream(client *http.Client, upstream config.Upstream, requestBody []byte, ctx context.Context) (*http.Response, error) {
	...
	return forwardStreamWithAuthRetry(authRetryConfig[AntigravityTokenStorage]{
		Label:   "[ANTIGRAVITY]",
		Manager: antigravityAuthManager,
		Validate: func(storage *AntigravityTokenStorage, authFile string) error {
			if storage.ProjectID == "" {
