...
func (h *ResponsesHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[RESPONSES REQUEST] Endpoint: %s, Model: %s, Stream: %v, Prompt: %s", r.URL.Path, model, stream, promptPreview)
	...
	for { // outer fallback loop
...

		{
			...
			for _, upstream := range ordered {
				...
				status, respBody, respHeaders, streamResp, err := h.forwardRequest(upstream, mappedModel, body, stream, r)
				if err != nil {
	...
	tryFallback:
		...
		var bodyMap2 map[string]any
		if err := json.Unmarshal(body, &bodyMap2); err == nil {
			bodyMap2["model"] = currentModel
			if newBody, err := json.Marshal(bodyMap2); err == nil {
				body = newBody
			}
		}
	}
...
func (h *ResponsesHandler) forwardRequest(upstream config.Upstream, model string, originalBody []byte, clientWantsStream bool, originalReq *http.Request) (int, []byte, http.Header, *http.Response, error) {
	...
	if apiType == config.APITypeResponses {
		...
		url := strings.TrimSuffix(upstream.BaseURL, "/") + originalReq.URL.Path
		rawQuery := ""
	...
	if clientWantsStream {
		if streamSender, ok := sender.(StreamCapableSender); ok {
			...
			if respFormat == config.APITypeResponses {
				log.Printf("[RESPONSES] stream_mode=format_compatible_passthrough upstream=%s", upstream.Name)
				return HandleStreamResponse(resp)
			}
			// Format mismatch: read body and fall through to conversion path
			log.Printf("[RESPONSES] stream_mode=buffered_conversion upstream=%s respFormat=%s", upstream.Name, respFormat)
			defer resp.Body.Close()
...
func init() {
	RegisterOutboundSender(config.APITypeResponses, func() OutboundSender { return &ResponsesSender{} })
}
